...
@DATASETS.register_module()
class CocoHRContDataset(CocoDataset):
    def __init__(self,
                 ann_file,
                 pipeline,
                 pre_pipeline=None,
        ...
        super(CocoHRContDataset, self).__init__(ann_file,
                 pipeline,
                 classes,
                 data_root,
                 img_prefix,
        ...
        if not self.test_mode:
            ...
            for pipe_student in pipeline:
                pipe_teacher = deepcopy(pipe_student)

                if pipe_student['type'] == 'Resize':
                    # Check Single or Multi-scale
                    if type(pipe_student['img_scale']) == tuple:
                        pipe_teacher['img_scale'] = (pipe_student['img_scale'][0] * 2, pipe_student['img_scale'][1] * 2)
                    else:
                        if len(pipe_student['img_scale']) == 1:
                            pipe_teacher['img_scale'] = [(pipe_student['img_scale'][0] * 2, pipe_student['img_scale'][1] * 2)]
                        else:
                            pipe_teacher['img_scale'] = []
                            for scale in pipe_student['img_scale']:
                                pipe_teacher['img_scale'].append((scale[0]*2, scale[1]*2))
