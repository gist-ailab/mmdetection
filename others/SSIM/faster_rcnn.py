...
class FasterRCNN_TS_SSIM_SCALE(TwoStageDetector):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(FasterRCNN_TS_SSIM_SCALE, self).__init__(
            backbone=backbone,
            neck=neck,
            rpn_head=rpn_head,
            roi_head=roi_head,
    ...
    def train_step(self, data, optimizer):
        ...
        ## SSIM loss
        if 'SSIM' in self.distill_name:
            from kornia.losses import ssim_loss
            loss_distill_ssim = 0.
            for backbone_down_ix, backbone_teacher_down_ix in zip(backbone_down, backbone_teacher_down):
                backbone_down_ix, backbone_teacher_down_ix = self.min_max(backbone_down_ix), self.min_max(backbone_teacher_down_ix)
                loss_distill_ssim += ssim_loss(backbone_down_ix, backbone_teacher_down_ix, window_size=11)
            
            loss_distill_ssim = loss_distill_ssim * 2.0
