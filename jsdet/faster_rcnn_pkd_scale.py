...
class FasterRCNN_PKD_SCALE(TwoStageDetector):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(FasterRCNN_PKD_SCALE, self).__init__(
            backbone=backbone,
            neck=neck,
            rpn_head=rpn_head,
            roi_head=roi_head,
    ...
    def train_step(self, data, optimizer):
        ...
        pkd_loss = 0.
        #####################################PKD######################################
        for backbone_down_, backbone_teacher_down in zip(backbone_down, backbone_teacher_down):
            size_S, size_T = backbone_down_.shape[2:], backbone_teacher_down.shape[2:]
            if size_S[0] != size_T[0]:
                if self.resize_stu:
                    backbone_down_ = F.interpolate(backbone_down_, size_T, mode='bilinear')
                else:
                    backbone_teacher_down = F.interpolate(backbone_teacher_down, size_S, mode='bilinear')
            assert backbone_down_.shape == backbone_teacher_down.shape
            norm_S, norm_T = self.norm(backbone_down_), self.norm(backbone_teacher_down)
            pkd_loss += F.mse_loss(norm_S, norm_T) / 2
        losses.update({'pkd_loss': pkd_loss})
        #####################################################################################
